...
elif [ ! -d "iti0203-2019-front" ]; then
  printf "${YELLOW}cloning git repo${NC}\n"
  git clone https://gitlab.cs.ttu.ee/envomp/iti0203-2019-front.git
  cd iti0203-2019-front
...
#ACQUIRE IP FOR THE DOCKER CONTAINER NAMED "front"
IP=$(sudo docker ps --filter "name=front" | grep -P "(\d.\d.\d.\d:\d+)" -o)
printf "${GREEN}Opening site at ${IP}.${NC}\n"
#OPEN THE SITE IN BROWSER WITH THE PREVIOUSLY ACQUIRED IP
xdg-open "http://${IP}"
